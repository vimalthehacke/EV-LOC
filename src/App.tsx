...
function PasswordGate({ onSuccess }: PasswordGateProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (password === "EVloc" || password === "EVLOC" || password === "evloc") {
      onSuccess();
    } else {
...
export default function App() {
  ...
  // Hidden admin panel paths conditions & authentication state
  const [isAdminRoute, setIsAdminRoute] = useState(false);
  const [isAdminUnlocked, setIsAdminUnlocked] = useState(() => {
    if (typeof window !== "undefined") {
      return sessionStorage.getItem("zonedel-admin-auth") === "unlocked";
    }
    return false;
  });
  ...
  // Isolate normal tracking viewers from administrative dashboards
  if (!isAdminRoute) {
    return (
      <div className="min-h-screen bg-[#030712] text-slate-100 font-sans flex flex-col items-center justify-center p-4 selection:bg-emerald-500 selection:text-slate-950">
        <UserBeacon onLocationLogged={handleLocationLogged} />
        {/* Absolute clean, unclickable, unbranded simple footnote */}
        <div className="mt-8 text-[9px] text-slate-700 font-mono tracking-widest uppercase select-none">
          SYSTEM LAYER SECURED BY SSL CERTIFICATE • ZONEDEL COMPANY 2026
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-slate-300 font-sans selection:bg-cyan-500 selection:text-slate-900 pb-12">
      
      {/* ⚠️ TOP EMERGENCY ALERT MARQUEE BANNER */}
      <div 
        className="bg-slate-950 border-b border-cyan-800/20 px-4 py-2 text-center flex items-center justify-center space-x-2.5 text-[10px] text-cyan-400 uppercase font-mono tracking-widest leading-none select-none"
      >
        <ShieldAlert className="h-3.5 w-3.5 text-cyan-400 animate-pulse" />
        <span>CLASSIFIED INTERNAL CONTROL CONSOLE • ZONEDEL DISPATCH COMMAND SYSTEM</span>
        <span className="hidden md:inline-block w-1.5 h-1.5 rounded-full bg-cyan-400 animate-ping"></span>
        <span className="hidden md:flex text-slate-500 items-center justify-center space-x-1.5">
          <Wifi className="h-3 w-3 inline text-emerald-500" />
          <span>LIVE WEB INTEGRATION ACTIVE</span>
        </span>
      </div>

      {/* TACTICAL HEADER PANEL */}
      <header className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-2">
        <div className="bg-slate-950 border border-cyan-500/20 rounded-xl p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4 shadow-[0_0_30px_rgba(6,182,212,0.03)] backdrop-blur-md">
          
          <div className="flex items-center space-x-3.5 select-none">
            <div className="p-3 bg-cyan-500 rounded-lg flex items-center justify-center text-slate-950 shadow-[0_0_15px_rgba(6,182,212,0.3)]">
              <Radio className="h-6 w-6 animate-pulse" />
            </div>
            <div>
              <div className="flex items-center space-x-2">
                <h1 className="text-xl font-black tracking-tighter text-white font-sans uppercase">
                  ZONEDEL <span className="text-cyan-400">DISPATCH</span> HUB
                </h1>
              ...
              <p className="text-[10px] font-mono text-slate-500 tracking-widest mt-0.5 uppercase">
                COURIER REAL-TIME LOCALIZATION SYSTEMS
              </p>
            </div>
          </div>

          {/* Quick Metrics & Dynamic Clock */}
          <div className="flex items-center justify-between md:justify-end gap-6 font-mono border-t border-slate-900 md:border-t-0 pt-3 md:pt-0">
            <div className="hidden sm:block text-right">
              <span className="block text-[9px] text-slate-500 uppercase">COMMAND STATE</span>
            ...
            <div className="text-right">
              <span className="block text-[9px] text-slate-500 uppercase text-right">GLOBAL PULSE</span>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 space-y-6">
        ...
        {/* 🎛️ PRIMARY TAB CONTROLS */}
        <div className="flex bg-slate-950 p-1 rounded-lg border border-slate-900 max-w-md mx-auto shadow-xl">
          <button
            type="button"
        ...
        <div className="transition-all duration-300">
          {activeTab === "user" ? (
            ...
            <PasswordGate onSuccess={() => {
              sessionStorage.setItem("zonedel-admin-auth", "unlocked");
        ...
        <div className="bg-slate-950 border border-slate-900 rounded-xl p-5 shadow-2xl space-y-3 backdrop-blur-md">
          <div className="flex items-center justify-between border-b border-slate-900 pb-2.5">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-xs font-bold font-mono text-slate-400 tracking-widest uppercase">
                ZONEDEL SYSTEM EMISSION FEEDS
              </h3>
            </div>
            <div className="text-[9px] font-mono text-slate-600 uppercase tracking-wider">
              IN-MEMORY LIVE STREAMING
            </div>
          </div>

          <div className="bg-slate-950 rounded-lg p-4 font-mono text-xs text-slate-400 space-y-1.5 max-h-[140px] overflow-y-auto border border-slate-900/60 select-text">
            {systemLogs.length > 0 ? (
              systemLogs.map((log) => (
                <div key={log.id} className="flex items-start space-x-2 text-[11px] leading-relaxed">
                  ...
                  <span className={`px-1 rounded text-[9px] font-bold leading-none py-0.5 uppercase tracking-wider ${
                    log.type === "success" 
                      ...
                      : "bg-[#111827] text-cyan-300 border border-cyan-950"
                  }`}>
                    {log.type}
                  </span>
      ...
      <footer className="max-w-7xl mx-auto px-4 mt-8 text-center text-[10px] font-mono text-slate-600 uppercase tracking-widest">
        <p>© 2026 ZONEDEL LOGISTICS NETWORKS • PURE IN-MEMORY REALTIME BROADCAST ONLY</p>
        <p className="mt-1">SYSTEM INSTANCE: LIVE DISPATCH • TEMPORARY TELEMETRY COCKPIT</p>
      </footer>
